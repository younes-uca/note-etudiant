...
@Component({
  selector: 'app-semester-edit-admin',
  templateUrl: './semester-edit-admin.component.html'
})
export class SemesterEditAdminComponent extends AbstractEditController<SemesterDto, SemesterCriteria, SemesterService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.startDate = this.semesterService.format(this.item.startDate);
        this.item.endDate = this.semesterService.format(this.item.endDate);
    }
   ...
   public addModules() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.modulesElement.id == null){
                this.item.modules.push(this.modulesElement);
            }else{
                const index = this.item.modules.findIndex(e => e.id == this.modulesElement.id);
                this.item.modules[index] = this.modulesElement;
            }
    ...
    private validateModulesLibelle(){
