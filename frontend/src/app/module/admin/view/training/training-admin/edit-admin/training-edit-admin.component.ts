...
@Component({
  selector: 'app-training-edit-admin',
  templateUrl: './training-edit-admin.component.html'
})
export class TrainingEditAdminComponent extends AbstractEditController<TrainingDto, TrainingCriteria, TrainingService>   implements OnInit {
   ...
   public addStudents() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.studentsElement.id == null){
                this.item.students.push(this.studentsElement);
            }else{
                const index = this.item.students.findIndex(e => e.id == this.studentsElement.id);
                this.item.students[index] = this.studentsElement;
            }
    ...
    private validateStudentsFirstName(){
    ...
    private validateStudentsLastName(){
