...
export class StudentEditAdminComponent extends AbstractEditController<StudentDto, StudentCriteria, StudentService>   implements OnInit {
   ...
   public addModulesGrades() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.modulesGradesElement.id == null){
                this.item.modulesGrades.push(this.modulesGradesElement);
            }else{
                const index = this.item.modulesGrades.findIndex(e => e.id == this.modulesGradesElement.id);
                this.item.modulesGrades[index] = this.modulesGradesElement;
            }
